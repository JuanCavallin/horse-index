...
BEGIN
  ...
  -- AUDIT LOGS TABLE
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_trail') THEN
    EXECUTE format('
...
BEGIN
  ...
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_trail') THEN
    DROP POLICY IF EXISTS "Allow full access to audit_logs" ON public.audit_trail;
  END IF;
END $$;
...
-- Audit logs policies
DROP POLICY IF EXISTS "Anyone authenticated can view audit logs" ON public.audit_trail;
CREATE POLICY "Anyone authenticated can view audit logs" ON public.audit_trail
  FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Editors and admins can insert audit logs" ON public.audit_trail;
CREATE POLICY "Editors and admins can insert audit logs" ON public.audit_trail
  FOR INSERT WITH CHECK (public.get_user_role() IN ('editor', 'administrator'));
